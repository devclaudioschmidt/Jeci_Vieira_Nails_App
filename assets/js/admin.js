...
async function abrirModalDetalhesAgendamento(id) {
    ...
    const ehConfirmado = agendamento.status === 'confirmado';
    ...
    modal.querySelector('.modal-botoes').innerHTML = `
        ...
        ${!ehConfirmado && !ehCancelado && !ehPassado ? `
        <button class="modal-botao success" id="modal-confirmar-agend">Confirmar</button>
        ` : ''}
    ...
    // Evento Confirmar
    const btnConfirmar = document.getElementById('modal-confirmar-agend');
    if (btnConfirmar) {
